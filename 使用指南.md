...
1. 打开浏览器，访问：`https://你的网站域名/admin/rules`
   - 例如：`https://inspetionreport.netlify.app/admin/rules`
2. 首次访问会看到 **「Admin Token」** 输入框，输入与 Netlify 中 `ADMIN_TOKEN` 完全一致的密码
3. 点击 **「验证并进入」**，验证通过后进入规则管理页
4. Token 会保存在浏览器中，下次访问可自动登录

**若提示 "Unauthorized - Invalid token"：**
- 说明输入的 Token 与 Netlify 的 `ADMIN_TOKEN` 不一致
- 页面上有 **「清除已保存的 Token」** 按钮，点击后可重新输入
- 或清除浏览器该站点的 localStorage 后刷新页面，即可再次输入 Token
...
**问题**: 提示 "Unauthorized - Invalid token" 或无法再次输入 Token

**解决方案**:
1. 在规则管理登录页点击 **「清除已保存的 Token」**，然后重新输入正确的 Token
2. 或手动清除：打开开发者工具（F12）→ Application → Local Storage → 删除 `admin_token` → 刷新页面
3. 确认 Netlify 环境变量 `ADMIN_TOKEN` 已设置，且输入的 Token 与之**完全一致**（区分大小写、无多余空格）
4. 若**未**在 Netlify 设置 `ADMIN_TOKEN`，默认 Token 为：`admin-secret-token-change-me`
