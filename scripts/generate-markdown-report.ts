#!/usr/bin/env node
/**
 * 生成 Markdown 格式的报告，然后可以转换为 Word
 * 
 * 使用方法：
 *   npm run test:markdown
 *   或
 *   tsx scripts/generate-markdown-report.ts
 */
...
function generateMarkdownReport(templateData: any): string {
  ...
  md.push("");
  if (templateData.IMMEDIATE_FINDINGS && templateData.IMMEDIATE_FINDINGS !== "No immediate safety risks were identified at the time of inspection.") {
  ...
  md.push("");
  if (templateData.RECOMMENDED_FINDINGS && templateData.RECOMMENDED_FINDINGS !== "No items requiring short-term planned action were identified at the time of inspection.") {
  ...
  md.push("");
  if (templateData.PLAN_FINDINGS && templateData.PLAN_FINDINGS !== "No additional items were identified for planning or monitoring at this time.") {
...
function createTestInspection(): StoredInspection {
  // 读取 sample-inspection.json 作为基础
